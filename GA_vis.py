...
def FirstPopulation():
    ...
    for i in range(n):
        np.append(ArrPopulation, random.randrange(ValueFirst, ValueLast))
...
NewDecisions = np.array([], int) #массив новых особей
NewDecisionMut = np.array([], int) #одна новая особь
ArrPopulation = np.array([], int) #готовая популяция
